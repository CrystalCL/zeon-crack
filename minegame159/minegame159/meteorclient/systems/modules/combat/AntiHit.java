...
extends Module {
    ...
    public AntiHit() {
        ...
        this.entities = this.sgGeneral.add(new EntityTypeListSetting.Builder().name("entities").description("Entities to avoid attacking.").defaultValue((Object2BooleanMap<EntityType<?>>)new Object2BooleanOpenHashMap(0)).onlyAttackable().build());
    }

    @EventHandler(priority=100)
    private void onAttackEntity(AttackEntityEvent attackEntityEvent) {
        if (this.antiFriendHit.get().booleanValue() && attackEntityEvent.entity instanceof PlayerEntity && !Friends.get().attack((PlayerEntity)attackEntityEvent.entity)) {
            attackEntityEvent.cancel();
        }
        if (Modules.get().isActive(AntiHit.class) && this.entities.get().containsKey((Object)attackEntityEvent.entity.getType())) {
            attackEntityEvent.cancel();
        }
    }
}
