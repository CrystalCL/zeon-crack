...
public class CustomCrystalAuraV1 extends Module {
    ...
    public CustomCrystalAuraV1() {
        super(CrystalCL.Exc, "crystal-aura-V1", "Normal version.");
    }
                        ...
                        if (switchMode.get() != SwitchMode.None || mc.player.getMainHandStack().getItem() == Items.END_CRYSTAL || mc.player.getOffHandStack().getItem() == Items.END_CRYSTAL) {
                            if (place.get()) {
                                ...
                                if (surroundHold.get() && heldCrystal == null) {
                                    slot = InvUtils.find(Items.END_CRYSTAL).getSlot();
                                ...
                                if (spamface.get() && Math.sqrt(target.squaredDistanceTo(bestBlock)) <= 2.0D) {
                                    if ((target.getHealth() + target.getAbsorptionAmount()) < faceplacehealth.get()) {
                                        ...
                                        target = TargetUtils.getPlayerTarget(placeRange.get(), SortPriority.LowestDistance);
                                        if (target == null || mc.player.distanceTo(target) > (float) range.get()) {
                                            return;
                                        }
    ...
    private void findTarget() {
        ...
        target = livingEntity.orElse(null);
    }
    ...
    private void placeBlock(Vec3d block, Hand hand) {
        ...
        if (mc.world.isAir(new BlockPos(block)) && support.get() == SupportMode.Plus) {
            BlockUtils.place(new BlockPos(block), Hand.MAIN_HAND, supportSlot, rotationMode.get() == RotationMode.PlaceBreak || rotationMode.get() == RotationMode.Place, 50, true, false, true);
        ...
        if (render.get()) {
            ...
            renderBlock.damage = DamageUtils.crystalDamage(target1, bestBlock.add(0.5D, 1.0D, 0.5D));
            ...
            for (double j = playerPos.getZ() - placeRange.get(); j < playerPos.getZ() + placeRange.get(); j++) {
                ...
                for (double k = playerPos.getY() - verticalRange.get(); k < playerPos.getY() + verticalRange.get(); ++k) {
                    ...
                    if (isValid(new BlockPos(pos)) && getDamagePlace((new BlockPos(pos)).up()) && (!oldPlace.get() || isEmpty(new BlockPos(pos.add(0.0D, 2.0D, 0.0D)))) && (!rayTrace.get() || pos.distanceTo(new Vec3d(mc.player.getX(), mc.player.getY() + mc.player.getEyeHeight(mc.player.getPose()), mc.player.getZ())) <= placeWallsRange.get() || rayTraceCheck(new BlockPos(pos), false) != null)) {
                        if (!multiTarget.get()) {
                            if (isEmpty(new BlockPos(pos)) && bestSupportDamage < DamageUtils.crystalDamage(target1, pos.add(0.5D, 1.0D, 0.5D))) {
                                bestSupportBlock = pos;
                                bestSupportDamage = DamageUtils.crystalDamage(target1, pos.add(0.5D, 1.0D, 0.5D));
                            } else if (!isEmpty(new BlockPos(pos)) && bestDamage < DamageUtils.crystalDamage(target1, pos.add(0.5D, 1.0D, 0.5D))) {
                                bestBlock = pos;
                                bestDamage = DamageUtils.crystalDamage(target1, bestBlock.add(0.5D, 1.0D, 0.5D));
                            }
                        } else {
                            ...
                            while (true) {
                                ...
                                } while (!Friends.get().shouldAttack((PlayerEntity) entity));
