...
public class AutoCrystalHead extends Module {
    ...
    public AutoCrystalHead() {
        super(CrystalCL.Exc, "auto-crystal-head", "Auto Crystal Head.");
    }

    @EventHandler
    private void BlockUpdate(PacketEvent.Receive e) {
        if (e.packet instanceof BlockUpdateS2CPacket p) {
            if (Ezz.equalsBlockPos(p.getPos(), pos) && p.getState().isAir()) {
                isDone = true;
            }
        }
    }
    ...
    private void AntiClick(PacketEvent.Send e) {
        if (e.packet instanceof PlayerActionC2SPacket) {
            ...
            s(var10001 + " " + p.getPos());
            if ((p.getAction() == PlayerActionC2SPacket.Action.START_DESTROY_BLOCK || p.getAction() == PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK || p.getAction() == PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK) && !Ezz.equalsBlockPos(p.getPos(), pos)) {
                s("cancel!");
                e.cancel();
            }
        }
        ...
        if (mc.world != null && mc.player != null) {
            if (pause > 0) {
                ...
                if (player != null) {
                    ...
                    if (!(Ezz.DistanceTo(obsidianPos) > range.get())) {
                            ...
                            if (mc.world.getBlockState(obsidianPos).isAir() || mc.world.getBlockState(obsidianPos).getBlock() == Blocks.OBSIDIAN) {
                                ...
                                if (pickaxe == -1) {
                                    ...
                                    if (mc.world.getBlockState(obsidianPos).getBlock() != Blocks.OBSIDIAN) {
                                        Ezz.BlockPlace(obsidianPos, InvUtils.findInHotbar(Items.OBSIDIAN).getSlot(), rotate.get());
                                    }
    ...
    private boolean placeCrystal(PlayerEntity player, BlockPos obsidianPos) {
        ...
        if (!BlockUtils.canPlace(crystalPos, true)) {
            ...
            if (crystalSlot == -1) {
                error("There are no crystals in the quick access bar!");
                toggle();
                return false;
            } else {
