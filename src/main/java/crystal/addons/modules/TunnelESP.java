...
public class TunnelESP extends Module {
    ...
    public final Setting<SettingColor> color = sgGeneral.add(new ColorSetting.Builder().name("color").description("").defaultValue(new SettingColor(255, 0, 190, 255)).build());
    private final Setting<Integer> delay = sgGeneral.add(new IntSetting.Builder().name("delay").description("delay").defaultValue(1).min(0).sliderMax(20).build());
    private final Setting<Integer> range = sgGeneral.add(new IntSetting.Builder().name("range").description("Range").defaultValue(50).min(0).sliderMax(150).build());
    private final List<BlockPos> poses = new ArrayList();
    public Vec3d prevPos;
    private double[] rPos;

    public TunnelESP() {
        super(CrystalCL.PvE, "TunnelESP", "Tunnel ESP.");
    }
